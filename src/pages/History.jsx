...
// ── Daily Pick Row (with expand/collapse rationale) ───────────────────────────
function DailyPickRow({ pick, index, isSelected, onToggleLeg, ratingColor }) {
  const [expanded, setExpanded] = useState(false);
  const key = `${pick.player}:${pick.stat}`;
  const color = ratingColor(pick.rating);
  return (
    <div style={{ background: isSelected ? 'rgba(124,58,237,0.08)' : 'var(--bg-secondary, #111)', border: `1px solid ${isSelected ? '#7c3aed' : 'var(--border-color, #222)'}`, borderRadius: '16px', marginBottom: '10px', overflow: 'hidden', animation: `fadeUp 0.25s ease ${index * 0.04}s both` }}>
      <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div style={{ width: '42px', height: '42px', borderRadius: '12px', flexShrink: 0, background: `${color}15`, border: `1px solid ${color}30`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontSize: '17px', fontWeight: '500', color, lineHeight: 1 }}>{pick.rating}</span>
          <span style={{ fontSize: '9px', color, opacity: 0.8 }}>★</span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {expanded && (
        <div style={{ padding: '0 16px 14px', borderTop: '1px solid var(--border-color, #222)' }}>
          {pick.rationale && <p style={{ fontSize: '13px', color: 'var(--text-secondary, #999)', lineHeight: '1.6', margin: '10px 0 6px' }}>{pick.rationale}</p>}
          {pick.rating_reason && <div style={{ fontSize: '11px', color, fontWeight: '500' }}>{pick.rating_reason}</div>}
        </div>
      )}
    </div>
  );
...
export default function Halftime({ isDark, toggleTheme, onLogout }) {
  ...
  const scan = useCallback(async () => {
    if (mode === 'halftime') {
      setLiveScanState('scanning'); setErrorMsg(''); setLiveGames([]);
      try {
        const res  = await fetch('/api/halftime/scan?sports=nba,mlb');
        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data.error || 'Scan failed');
        setLiveGames(data.games); setLiveLast(new Date());
        setLiveScanState(data.games.length > 0 ? 'done' : 'empty');
      } catch (err) { setErrorMsg(err.message); setLiveScanState('error'); }
      return;
    }
    if (pregameSport === 'nba') {
      setNbaScanState('scanning'); setErrorMsg(''); setNbaGames([]);
      try {
        const res  = await fetch('/api/pregame/scan?sport=nba');
        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data.error || 'Scan failed');
        setNbaGames(data.games); setNbaOddsMap(data.oddsMap || {}); setNbaLastScanned(new Date());
        setNbaScanState(data.games.length > 0 ? 'done' : 'empty');
      } catch (err) { setErrorMsg(err.message); setNbaScanState('error'); }
    } else if (pregameSport === 'mlb') {
      setMlbScanState('scanning'); setErrorMsg(''); setMlbGames([]);
      try {
        const res  = await fetch('/api/pregame/scan?sport=mlb');
        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data.error || 'Scan failed');
        setMlbGames(data.games); setMlbLastScanned(new Date());
        setMlbScanState(data.games.length > 0 ? 'done' : 'empty');
      } catch (err) { setErrorMsg(err.message); setMlbScanState('error'); }
    } else {
      setNhlScanState('scanning'); setErrorMsg(''); setNhlGames([]);
      try {
        const res  = await fetch('/api/pregame/scan?sport=nhl');
        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data.error || 'Scan failed');
        setNhlGames(data.games); setNhlLastScanned(new Date());
        setNhlScanState(data.games.length > 0 ? 'done' : 'empty');
      } catch (err) { setErrorMsg(err.message); setNhlScanState('error'); }
    }
  }, [mode, pregameSport]);

  const switchMode = (newMode) => {
    setMode(newMode); setErrorMsg('');
    if (newMode === 'halftime') { setLiveScanState('idle'); setLiveGames([]); setLiveLast(null); }
  };

  const switchPregameSport = (sport) => { setPregameSport(sport); setErrorMsg(''); };
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        {mode !== 'daily' && mode !== 'performance' && (
          <div style={{ marginTop: '14px', padding: '10px 14px', background: 'var(--bg-secondary, #111)', border: '1px solid var(--border-color, #222)', borderRadius: '12px' }}>
            <style>{`
              .leg-slider { -webkit-appearance: none; appearance: none; width: 100%; height: 4px; border-radius: 2px; outline: none; cursor: pointer; background: linear-gradient(to right, #7c3aed ${(legCount - 2) / 3 * 100}%, var(--border-color, #333) ${(legCount - 2) / 3 * 100}%); }
