...
async function findGameIdForPlayer(config, playerName, gameDate) {
  // ESPN's scoreboard buckets games by US game-night date, not raw UTC date —
  // an evening tip-off's UTC timestamp often rolls into the next UTC day, so
  // a stored gameDate can land one calendar day ahead of ESPN's own bucket
  // for that same game. Check the day before too to cover that case.
  const primaryDateStr = formatDateForESPN(gameDate);
  const priorDateStr   = formatDateForESPN(shiftDate(gameDate, -1));

  const events = await fetchScoreboardEvents(config, [primaryDateStr, priorDateStr]);

  if (events.length === 0) {
    console.warn(`[espn-client] No events found for ${config.league} near ${primaryDateStr}`);
    return null;
  }

  // If only one game across both dates just return it (common for NFL)
...
// Fetches scoreboard events across multiple dates and dedupes by event id —
// used to search both a pick's stored date and the day before it.
async function fetchScoreboardEvents(config, dateStrs) {
  const seen = new Set();
  const all = [];
  for (const dateStr of dateStrs) {
    ...
    if (!res.ok) {
      console.error(`[espn-client] Scoreboard fetch failed: ${res.status} for ${dateStr}`);
      continue;
    }
