// FILE LOCATION: lib/mlb-odds.js
const ODDS_API_KEY = process.env.ODDS_API_KEY;
const ODDS_API_BASE = 'https://api.the-odds-api.com/v4';

// ESPN abbreviations that don't match Odds API team names
...
function resolveTeamName(name) {
  return ABBREV_TO_FULL[name?.toUpperCase()] || name;
}
...
export async function getGameOdds(homeTeam, awayTeam) {
  ...
  try {
    ...
    const event  = findEvent(events, resolveTeamName(homeTeam), resolveTeamName(awayTeam));
    if (!event) {
